...
import { ExpiresInDays, MediaRefType, UserTypes } from '@/constants'
...
export const userController = {
  ...
  verification: async (
    ...
    try {
      ...
      const { accessToken } = jwtSign(user.id, user.userType)
  ...
  updateUserTypeByMail: async (
    {
      body: { email, user_type }
    }: IBodyRequest<{ email: string; user_type: UserTypes }>,
    res: Response
  ) => {
    try {
      await userService.updateTypeByMail(email, user_type)
  ...
  adminCreateUser: async (
    { body }: IBodyRequest<AdminCreateUserPayload>,
    res: Response
  ) => {
    const session = await startSession()
    try {
      const isUserExist = await userService.isExistByEmail(body.email)
  ...
  // TEST-ONLY: Unsecured endpoint that accepts body or query params to create a user
  testAdminCreateUser: async (req: Request, res: Response) => {
    const session = await startSession()
    try {
      const src: any =
      ...
      const password = src?.password?.toString()
      const firstName =
        src?.firstName !== undefined ? src.firstName?.toString() : undefined
      const lastName =
        src?.lastName !== undefined ? src.lastName?.toString() : undefined
      const userType =
        src?.userType !== undefined ? src.userType?.toString() : undefined
      const subscriptionStatus =
        src?.subscriptionStatus !== undefined
          ? src.subscriptionStatus?.toString()
          : undefined

      const verified =
        src?.verified !== undefined
          ? typeof src.verified === 'string'
            ? src.verified.toLowerCase() === 'true'
            : !!src.verified
          : true
  ...
  adminUpdateUser: async (
    {
      body,
      params
    }: ICombinedRequest<IUserRequest, AdminUpdateUserPayload, { id: ObjectId }>,
    res: Response
  ) => {
    const session = await startSession()
    try {
      ...
      // If admin provided password, hash and update it, then update other fields
      const nonPasswordBody: any = { ...body }
      if (Object.prototype.hasOwnProperty.call(nonPasswordBody, 'password')) {
        const plain = nonPasswordBody.password
        delete nonPasswordBody.password
        if (plain !== undefined) {
          const hashedPassword = await createHash(plain)
      ...
      if (Object.keys(nonPasswordBody).length > 0) {
        await userService.updateById(params.id, nonPasswordBody, session)
      }

      const updated = await userService.getById(params.id)
